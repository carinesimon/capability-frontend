                ...
                rows.map((r) => (
                  <tr key={r.leadId + Math.random()} className="border-t border-white/10">
                    ...
                    <td className="py-2 pr-2">
                      {Number.isFinite(r.opportunityValue as number)
                        ? `${Math.round(Number(r.opportunityValue)).toLocaleString("fr-FR")} €`
                        : "—"}
                    </td>
                    <td className="py-2 pr-2">
                      {Number.isFinite(r.saleValue as number)
                        ? `${Math.round(Number(r.saleValue)).toLocaleString("fr-FR")} €`
                        : "—"}
                    </td>
                    <td className="py-2 pr-2 text-[--muted]">
                    ...
                    <td className="py-2 pr-2 text-[--muted]">
                      {r.stageUpdatedAt ? new Date(r.stageUpdatedAt).toLocaleString() : "—"}
                    </td>
                  </tr>
                ))
              ) : (
...
export default function ProspectsPage() {
  ...
  async function loadBoard() {
    const res = await api.get<BoardResponse>("/leads/board", {
      params: { from: fromISO, to: toISO, limit: 200 },
    });
    setBoard(res.data ?? ({ columns: {} } as any));
  }
  async function loadColumnsConfig() {
    try {
      const res = await api.get<{ ok: true; columns: ColumnConfig[] }>("/leads/columns-config");
  ...
  async function saveColumnsConfig(next: ColumnConfig[]) {
    ...
    await api.put("/leads/columns-config", payload);
  ...
  async function trackStageIncrement(leadId: string, stage: LeadStage) {
    ...
    const type = STAGE_TO_EVENT[stage] || stage;
    try {
      await api.post(`/leads/${leadId}/events`, {
        type,
        source: "board-drop",
  ...
  async function moveToColumn(target: ColumnConfig) {
    ...
    if (target.stage) {
      if (target.stage === "WON") {
        try {
          await api.patch(`/leads/${dragLead.id}/stage`, { stage: "WON" });
      ...
      try {
        await api.patch(`/leads/${dragLead.id}/stage`, { stage: target.stage });
    ...
    try {
      await api.patch(`/leads/${dragLead.id}/board-column`, { columnKey: target.id });
      await loadBoard();
    } catch (e: any) {
  ...
  async function confirmWonSame() {
    ...
    try {
      await api.patch(`/leads/${askWon.lead.id}/stage`, { stage: "WON", confirmSame: true });
  ...
  async function confirmWonWithValue() {
    ...
    try {
      await api.patch(`/leads/${askWon.lead.id}/stage`, { stage: "WON", saleValue: v });
      await trackStageIncrement(askWon.lead.id, "WON");
      setAskWon({ open: false, lead: null });
      await loadBoard();
    } catch (e: any) {
  ...
  async function saveEdit() {
    ...
    try {
      await api.patch(`/leads/${editLeadId}`, payload);
      setEditOpen(false);
      await loadBoard();
    } catch (e: any) {
  ...
  async function loadActors() {
    try {
      const res = await api.get<{ setters: UserMini[]; closers: UserMini[] }>("/leads/actors");
  ...
  async function createLead() {
    ...
    try {
      ...
      setErr(e?.response?.data?.message || "Impossible de créer le lead");
    }
  }
  ...
  const allStageItems = useMemo(() => {
    ...
    const mapped = enabledCols
      .filter((c) => c.stage)
      .flatMap((c) => cols[c.stage as LeadStage]?.items ?? []);
    const byKey = enabledCols
      .filter((c) => !c.stage && board?.extraByColumnKey?.[c.id])
      .flatMap((c) => board!.extraByColumnKey![c.id]);
    return [...mapped, ...byKey];
  }, [board, columns]);
  ...
  const kpi = useMemo(() => {
    ...
    if (!cols || !enabled.length)
      return { leads: 0, oppo: 0, sales: 0, wonCount: 0, conv: 0 };

    const mapped = enabled.filter((c) => c.stage) as Array<
      ColumnConfig & { stage: LeadStage }
    >;
  ...
  return (
    <div className="h-[100dvh] w-full">
            ...
            <div className="relative h-full">
              ...
              >
                <div className="grid grid-flow-col gap-3" style={scrollerStyle}>
                  {columns
                    ...
                    .map((col) => {
                      ...
                      const sOppoNum = items.reduce(
                        (s, l) => s + (l.opportunityValue ?? 0),
                        0,
                      );
                      ...
                      return (
                        ...
                        >
                          <div className="sticky top-0 -mx-3 px-3 py-2 bg-[rgba(16,22,33,.85)] backdrop-blur rounded-t-xl border-b border-white/10 z-[10]">
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2">
                                {mappedStage && (
                                  <span
                                    className={`inline-block w-2.5 h-2.5 rounded-full ${STAGE_DOT[mappedStage]}`}
                                  />
                                )}
                          ...
                          <div
                            ...
                            } max-h-[66vh] overflow-y-auto overflow-x-hidden pr-1 pb-1`}
                          >
                            {loading && items.length === 0 &&
                              Array.from({ length: 6 }).map((_, i) => (
                                <div
                                  key={i}
                                  className={`rounded-xl border border-white/10 bg-white/5 ${cardPad} animate-pulse`}
                                >
                            ...
                            {items.map((lead) => (
                              ...
                              >
                                {mappedStage && (
                                  <div
                                    className={`absolute left-0 top-0 bottom-0 w-1 rounded-l-xl bg-gradient-to-b ${STAGE_COLOR[lead.stage]}`}
                                  />
                                )}
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-2">
                                    {lead.tag && (
                                      <span
                                        className={`${infoSize} px-2 py-0.5 rounded-lg bg-white/10`}
                                      >
                                        {lead.tag}
                                      </span>
                                    )}
                                    {lead.source && (
                                      <span
                                        className={`${infoSize} px-2 py-0.5 rounded-lg bg-white/10 opacity-80`}
                                      >
                                        {lead.source}
                                      </span>
                                    )}
                                ...
                                {/* Infos */}
                                <div
                                  className={`mt-1 ${infoSize} text-[--muted] break-all truncate`}
                                >
                                  {lead.email || "—"} • {lead.phone || "—"}
                                </div>
                                <div
                                  className={`mt-1 flex items-center gap-2 ${infoSize} text-[--muted]`}
                                >
                                  {lead.setter && (
                                    <span className="px-2 py-0.5 rounded bg-white/10">
                                      Setter: {lead.setter.firstName}
                                    </span>
                                  )}
                                  {lead.closer && (
                                    <span className="px-2 py-0.5 rounded bg-white/10">
                                      Closer: {lead.closer.firstName}
                                    </span>
                                  )}
                                </div>
                                ...
                                <div className="mt-2 text-sm flex items-center justify-between">
                                  {mappedStage === "WON" ? (
                                    <span className="font-semibold">
                                      {fmtEUR(lead.saleValue)}
                                    </span>
                                  ) : (
                                    <span className="text-[--muted]">
                                      Oppo: {fmtEUR(lead.opportunityValue)}
                                    </span>
                                  )}
                                </div>
                                ...
                                <div
                                  className="absolute inset-x-0 bottom-0 px-2 py-2 bg-gradient-to-t from-black/50 to-transparent backdrop-blur-sm
                ...
                <div className="mt-2 h-1 bg-white/10 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-white/40 transition-[width]"
      ...
      <AnimatePresence>
        {filtersOpen && (
          <motion.div
            className="fixed inset-0 z-[90] bg-black/40 backdrop-blur-sm flex items-start justify-end"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <button className="btn btn-ghost" onClick={() => setFiltersOpen(false)}>
                  Fermer
                </button>
              </div>
            ...
            >
              ...
              <div className="sticky top-0 z-10 px-5 py-3 border-b border-white/10 bg-[rgba(16,22,33,.98)] rounded-t-2xl">
                <div className="flex items-center justify-between">
                  ...
                  <button className="btn btn-ghost" onClick={cancelManage}>
                    Fermer
                  </button>
                </div>
                <div className="text-xs text-[--muted] mt-1">
                  Les colonnes <b>libres</b> (sans “Stage mappé”) n’affichent des leads que si tu les
                  y déposes (drag &amp; drop). Les colonnes avec “Stage mappé” synchronisent le{" "}
                  <code>stage</code> réel du lead.
                </div>
              </div>
                      ...
                      <div className="col-span-4">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setManageDraft(
                              manageDraft.map((x) => (x.id === c.id ? { ...x, label: v } : x)),
                            );
                          }}
                        />
                      </div>
              ...
              <div className="sticky bottom-0 z-10 px-5 py-3 border-t border-white/10 bg-[rgba(16,22,33,.98)] rounded-b-2xl">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button className="btn btn-primary" onClick={saveManage}>
                      Enregistrer
                    </button>
                  </div>
                </div>
      ...
      <AnimatePresence>
        {importOpen && (
          <motion.div
            className="fixed inset-0 z-[95] flex items-center justify-center bg-[var(--bg)]"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            >
              ...
              <div className="text-sm text-[--muted] mb-3">
                Colonnes recommandées : firstName, lastName, email, phone, tag, source,
                opportunityValue, setterEmail, closerEmail.
              </div>
              <form
                onSubmit={async (e) => {
                  e.preventDefault();
                  ...
                  try {
                    ...
                    fd.append("file", f);
                    await api.post("/leads/import-csv", fd, {
        ...
        {askWon.open && askWon.lead && (
          ...
          >
            ...
            >
              ...
              <div className="text-sm text-[--muted] mb-3">
                Cette valeur impacte le ROAS du closer, du setter et de l’entreprise.
              </div>
                  ...
                  <input
                    ...
                    placeholder={
                      askWon.lead.opportunityValue != null
                        ? String(askWon.lead.opportunityValue)
                        : "Ex: 5000"
                    }
                  />
                </div>
                <div className="flex gap-2">
                  <button className="btn btn-primary flex-1" onClick={confirmWonWithValue}>
                    Enregistrer
                  </button>
                  {askWon.lead.opportunityValue != null && (
      ...
      <AnimatePresence>
        {newOpen && (
          <motion.div
            className="fixed inset-0 z-[90] flex items-center justify-center bg-[var(--bg)]"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    className="input"
                    value={newData.tag}
                    onChange={(e) => setNewData({ ...newData, tag: e.target.value })}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewData({ ...newData, opportunityValue: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <select
                    className="input"
                    value={newData.setterId}
                    onChange={(e) => setNewData({ ...newData, setterId: e.target.value })}
                  >
                    <option value="">— Aucun —</option>
                    {actors?.setters?.map((u) => (
                ...
                <div>
                  ...
                  <select
                    className="input"
                    value={newData.closerId}
                    onChange={(e) => setNewData({ ...newData, closerId: e.target.value })}
                  >
                    <option value="">— Aucun —</option>
                    {actors?.closers?.map((u) => (
                      <option key={u.id} value={u.id}>
                        {u.firstName} ({u.email})
                      </option>
                    ))}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <button className="btn btn-ghost px-2 py-1" onClick={() => setEditOpen(false)}>
                  Fermer
                </button>
              </div>
              ...
              {editLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <div className="label">Prénom</div>
                    <input className="input" value={fFirst} onChange={(e) => setFFirst(e.target.value)} />
                  </div>
                  <div>
                    <div className="label">Nom</div>
                    <input className="input" value={fLast} onChange={(e) => setFLast(e.target.value)} />
                  </div>
                  <div>
                    <div className="label">Email</div>
                    <input className="input" value={fEmail} onChange={(e) => setFEmail(e.target.value)} />
                  </div>
                  <div>
                    <div className="label">Téléphone</div>
                    <input className="input" value={fPhone} onChange={(e) => setFPhone(e.target.value)} />
                  </div>
                  <div>
                    <div className="label">Tag</div>
                    <input className="input" value={fTag} onChange={(e) => setFTag(e.target.value)} />
                  </div>
                  <div>
                    <div className="label">Valeur d’opportunité (€)</div>
                    <input
                      ...
                      value={fOppo}
                      onChange={(e) => setFOppo(e.target.value)}
                      placeholder="Ex: 5000"
                    />
                  </div>
                  <div>
